...
impl LotteryConfigStore {
    ...
    /// 读取指定账号的开奖配置；未设置时返回默认行（`current_issue = 0`）。
    pub async fn get(&self, uid: i64) -> sqlx::Result<LotteryConfigRow> {
        let row = sqlx::query(
            "SELECT uid, api_url, current_issue, updated_at FROM lottery_config WHERE uid = ?",
        )
        .bind(uid)
    ...
    pub async fn upsert(&self, config: &LotteryConfigRow) -> sqlx::Result<()> {
        sqlx::query(
            r#"INSERT INTO lottery_config (uid, api_url, current_issue, updated_at)
               VALUES (?, ?, ?, ?)
